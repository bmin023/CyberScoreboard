...
#matchdesc.py
#It querys a minecraft server and then checks if the MOTD is the same as the one
...
#Pulled off the minecraft wiki. Not sure why I didn't look there earlier.
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 2:
        #The rest of the arguments are the string to check
    ...
    elif 'text' in status['description']:
        desc = status['description']['text']

    if stringToCheck in desc:
        print("Server is running:", desc)
        exit(0)
    else:
        print("Server running, but doesn't match desription:", desc)
        exit(1)
