            ...
            {injects.map((inject) => (
                <div key={inject.uuid} className="m-1">
                    <details>
                        ...
                        <div className="pl-7">
                            <InjectEditor inject={inject} onSubmit={()=>{
                                editInject(inject);
                            }}/>
                        </div>
                    </details>
...
const InjectEditor = ({ inject, onSubmit }: InjectEditorProps) => {
    ...
    const hasChanged = () => {
        const sideEffectsEqual = sideEffects.length === sideEffectData.length && sideEffects.every((sideEffect, i) => {
        ...
        <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-4">
                ...
                <label className="font-medium">Duration
                    <input type="number" className="w-20 mx-2 rounded bg-slate-100 px-1 shadow font-medium dark:bg-zinc-700" value={duration} onChange={e => setDuration(parseInt(e.currentTarget.value))} />
                    <span className="font-light">min</span>
                </label>
            </div>
            <div className="flex gap-2">
                ...
                <form onSubmit={addFileType} className="focus-within:outline outline-1 bg-slate-100 rounded shadow-md px-1">
                    <input pattern="([Aa]ny|\.?\w+|\*)" name="filetype" placeholder="Add (e.g. .txt, .docx, any)" className="px-1 flex-grow bg-transparent outline-none" />
                    <input type="submit" value="+" />
                </form>
            </div>
            <details>
                <summary className="font-medium">Inject Description</summary>
                <textarea className="h-80 resize-y w-full rounded bg-slate-100 px-1 shadow font-medium dark:bg-zinc-700" value={markdown} onChange={e => setMarkdown(e.currentTarget.value)} />
            </details>
            <details>
                ...
                <div className="pl-5">
                    ...
                    <button onClick={addSideEffect} className="bg-slate-200 w-full px-1 rounded shadow-sm hover:shadow-md active:shadow-sm">Add Side Effect</button>
                    <div className="font-extralight">
                        Please know what you are doing if you are editing these. It is still suggested you use the config file.
                    </div>
                </div>
            </details>
            <button disabled={!hasChanged()} onClick={submit} className="bg-slate-200 w-full px-1 rounded shadow-sm hover:shadow-md active:shadow-sm disabled:shadow-none disabled:bg-opacity-50 disabled:text-slate-400">Save</button>
        </div>
    );
}
...
const FileTypeBadge = ({ filetype, onClick }: { filetype: string, onClick: () => void }) => {
    return (
        <button onClick={onClick} className="hover:shadow-md active:shadow-none bg-slate-100 px-1 rounded shadow-sm">{filetype}</button>
    );
}
...
export const SideEffectEditor = ({ sideEffect, onChange, onDelete, id }: SideEffectEditorProps) => {
    ...
    return (
        <div className="flex">{id}
            <input className="outline-none bg-slate-300 font-bold w-[7.5rem]" value={key} onChange={(e) => { setKey(e.currentTarget.value); }} /><b>:</b>
            <input className="outline-none flex-grow bg-slate-300 px-1 dark:bg-zinc-700" value={value} onChange={e => {
